...
That output is the real window from the "real grade" repository, run against the commit that introduced it: two concurrent evaluations both pass the lookup at line 51. Across that repository's 858 functions the scanner lists 9 windows; four are a filesystem check before an await in its setup commands.
