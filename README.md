...
This repository is set up the same way: [`.claude/pr-grade.json`](.claude/pr-grade.json) marks the checker, the selector, the skill, and the grade agent as silent-failure files, and [`.github/workflows/pr-grade.yml`](.github/workflows/pr-grade.yml) runs the check on every pull request except Dependabot's.
