...
A proof copy is a clone of the head commit outside the repository, with its installed dependencies linked in, one per grader. A grader writes a failing test there and runs it without setup, and nothing is registered in the repository. Name any dependency beyond `node_modules` and `.venv` under `proofDir` in `.claude/pr-grade.json`. The prep ends with a `merge:` line, which section 5 runs, and a `remove:` line to run once every grader has reported. In-thread there are no reports to merge, and only the `remove:` line is printed. A branch that changed a dependency's pin files gets no linked dependency: the prep names the install to run in each copy instead, since what the author installed may predate the change. A grader told where nothing is searches the filesystem for it. Graders who did spent up to eight of their fifty turns on it, and read three different installed copies of this file.
